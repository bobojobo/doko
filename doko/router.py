...
@router.post("/game/{id}/hand/order/", response_class=HTMLResponse)
async def game_hand_order_post(
    id: str,
    data: request_dto.GameHandOrder,
    session: db.AsyncSession = Depends(db.session),
    session_token: str = Cookie(),
) -> None:
    """Update the order of cards in the player's hand."""

    await logic.game.update_hand_order(session=session, session_token=session_token, data=data, game_id=id)
