...
async def update(session: AsyncSession, session_token: str, game_id: str) -> response_dto.GameReviewUpdate:
    ...
    logging.info(f"{user.name}: all_ready={all_ready}")

    if all_ready:
        # START!
        leader = await group.leader()
        # Check if group has an active sitting and if that sitting needs a new game
        assert await group.has_active_sitting(session=session), "Group should have an active sitting"
        ...
        if (user.id == leader.id) and group_needs_active_game:
            ...
            logging.info(f"{user.name}: created new game {new_game_id}")
        else:
            logging.info(f"{user.name}: waiting for game_created event")
            async for _ in sse.EventLoop(session_token, sse.Event.game_created):
                new_game = await player.get_active_game(session=session)
                new_game_id = str(new_game.id)
                logging.info(f"{user.name}: received game_created event, game_id={new_game_id}")
                break
...
async def ready(data: request_dto.GameReviewReady, session: AsyncSession, session_token: str) -> None:
    ...
    # Check if all players are ready and if this player should create the game
    all_ready = await group.all_players_are_waiting()
